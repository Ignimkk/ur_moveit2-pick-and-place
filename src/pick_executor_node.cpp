...
: Node("pick_executor_node", options)
{
  ...
  cmd_sub_ = this->create_subscription<std_msgs::msg::String>(
    "/pick_executor_node/cmd", 10,
    std::bind(&PickExecutorNode::cmdCallback, this, std::placeholders::_1));

  // Parameter callback 등록 (런타임에 파라미터 변경 가능)
  param_callback_handle_ = this->add_on_set_parameters_callback(
    std::bind(&PickExecutorNode::parametersCallback, this, std::placeholders::_1));
  
  RCLCPP_INFO(this->get_logger(), "Pick Executor Node initialized with topic-based pause/resume support");
  ...
  while (true) {
    ...
    if (use_cartesian_path_) {
      ...
      double fraction = move_group_arm_->computeCartesianPath(
          waypoints, eef_step, 0.0, cartesian_trajectory);
...
bool PickExecutorNode::ascendFromTarget(const geometry_msgs::msg::Pose & target_pose)
{
  ...
  double fraction = move_group_arm_->computeCartesianPath(
      retreat_waypoints, eef_step, 0.0, trajectory_retreat);
...
void PickExecutorNode::cmdCallback(const std_msgs::msg::String::SharedPtr msg)
{
