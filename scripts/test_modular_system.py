...
class PickPlaceTester(Node):
    def __init__(self):
        ...
        # Publisher - Pick goal만 전송 (Place는 하드코딩됨)
        self.pick_goal_pub = self.create_publisher(PoseStamped, '/pick_goal', 10)
        
        self.get_logger().info('Pick Place Tester initialized (place is hardcoded)')
        
    def send_pick_goal(self, x=0.010, y=0.410, z=0.264):
        """Pick 위치 목표 전송 (Place는 하드코딩됨: x=-0.340, y=0.310, z=0.264)"""
        pose = PoseStamped()
        ...
        self.get_logger().info('Place goal is hardcoded: x=-0.340, y=0.310, z=0.264')
...
def main(args=None):
    ...
    try:
        ...
        tester.get_logger().info('=== Auto Pick and Place Sequence (Hardcoded Place) ===')
        
        # Pick 목표 설정 (Place는 하드코딩됨)
        tester.get_logger().info('Setting pick goal (place is hardcoded)...')
        tester.send_pick_goal()
        time.sleep(1)
        
        tester.get_logger().info('Pick goal sent - sequence will start automatically with hardcoded place')
